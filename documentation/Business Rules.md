...
- **Single serialized write path**: Every settings mutation — tab controls and the model commands alike — goes through `updateSettings(mutator)`: persist-then-commit (memory swaps only after `saveData()` succeeds) and serialized (each mutation derives from the previously committed state). The Ollama service re-reads its config strictly AFTER a successful commit; the old `saveSettings` broadcast unconfirmed state.
